...
class UserBaggageEqModel extends Model
{
	...
	public function equipNewEq($u_id,$b_equ_id,$equ_part,$w_id,$user_beq_id){
		$now=new DateTime;
		$datetime=$now->format( 'Y-m-d h:m:s' );

		$this->where('u_id',$u_id)->where('status',1)->where('b_equ_type',$equ_part)->('user_beq_id',$w_id)->update(['status'=>0,'updated_at'=>$datetime]);
