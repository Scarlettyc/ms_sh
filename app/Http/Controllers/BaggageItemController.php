		...
		$baggage_id=$data['baggage_id'];
			...
			if($validate&&$scrollQu){
				if($scrollInfo['equ_group']==0){
					$equipmentInfo=$EquipmentMstModel->select('equ_id','equ_rarity','equ_type','icon_path')->where('equ_rarity',$scrollInfo['sc_rarity'])->orderBy(DB::raw('RAND()'))->take(10)->first();
				}
				else{
					$equipmentInfo=$EquipmentMstModel->select('equ_id','equ_rarity','equ_type','icon_path')->where('equ_rarity',$scrollInfo['sc_rarity'])->where('equ_group',$scrollInfo['equ_group'])->		orderBy(DB::raw('RAND()'))->take(10)->first();
					}
