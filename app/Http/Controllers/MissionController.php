...
class MissionController extends Controller
{
	public function getMission(Request $request){
		...
		foreach ($missionList as $key => $mission) {
			...
			$rewards=$missionReward->select('item_id', 'equ_type','item_quantity', 'item_rarilty', 'item_type','if(item_type=3, CONCAT('sc_img_path',item_rarilty),'') as sc_path')->where('mission_id',$mission['mission_id'])->Get();
