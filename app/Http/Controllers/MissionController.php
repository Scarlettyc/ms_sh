...
class MissionController extends Controller
{
	public function getMission(Request $request){
		...
		$mission_key='mission_daily_'.$dmy.'_'.$u_id;
			...
			$rewards=$missionReward->select(DB::raw('item_id, equ_type,item_quantity, item_rarilty, item_type,if(item_type=2, CONCAT("Scroll_Random_",item_rarilty),"") as sc_img_path'))->where('mission_id',$mission['mission_id'])->Get();
			...
			foreach ($missionList as $key => $mission) {
				...
				else if($user_lv>=$mission['user_lv_from']&&$mission_type==1){
					$tmp['status']=1;
				}
				else {
					$tmp['status']=0;
