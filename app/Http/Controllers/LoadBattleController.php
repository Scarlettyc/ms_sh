...
class LoadBattleController extends Controller
{
    ...
    private function getData($u_id){
        ...
        $moveData=$eqModel->select('special_skill_id')->where('equ_id',$movement_id)->first();
 	    $result=[];
        $result['special_skill']=$skillModel->select('skill_id','skill_group', 'skill_name','skill_icon','skill_info')->where('skill_id',$eqData['special_skill_id'])->first();
        $result['normal_skill']=$skillModel->select('skill_id','skill_group', 'skill_name','skill_icon','skill_info')->where('skill_id',$eqData['normal_skill_id'])->first();
