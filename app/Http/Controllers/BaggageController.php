...
class BaggageController extends Controller
{
	...
	public function sellItem (Request $request)
	{
		...
		{
			$UserBaggageEqModel->where('u_id',$u_id)->where('status','=',0)->where('b_equ_id',$ItemId)->limit(1)->update(['status'=>1,'updated'=>$datetime]);
		...
		{
			$UserBaggageScrollModel->where('u_id',$u_id)->where('status','=',0)->where('bsc_id',$ItemId)->limit(1)->update(['status'=>1,'updated'=>$datetime]);
			$UserCoin=$UserModel->where('u_id',$u_id)->pluck('u_coin');
			$updateCoin=$UserCoin+$ItemPrice;
			$UserModel->where('u_id',$u_id)->update(['u_coin'=>$updateCoin,'updated_at'=>$datetime]);
